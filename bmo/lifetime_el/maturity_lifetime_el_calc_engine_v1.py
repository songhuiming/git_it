...
detailed_calc_result = pd.DataFrame(index = range(1, term_month + 1), columns = common_columns)
for monthIndex in range(1, term_month + 1):
	if monthIndex == 1:
		# D: starting_balance
...
effective_life = sum(selected_data.discount_cashflow * selected_data.monthIndex)	/ sum(selected_data.discount_cashflow)
total_loss_w_discount = sum(selected_data.discount_loss)

# calculate by prorate pd
lifetime_pd_lookup = final_cum_pd.ix[np.digitize([effective_life], final_cum_pd.month_i) - 1, 'cumulative_pd']
